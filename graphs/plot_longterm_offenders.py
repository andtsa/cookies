...
Usage:
    python graphs/plot_longterm_offenders.py --data cookies_data --out plots/cookie_lifetime --top_n 25
"""
...
def plot_offenders(data_dir: str, out_dir: str, top_n: int = 25):
    ...
    persistent["long_lived"] = (
        persistent["lifetime_days"] > LONGTERM_THRESHOLD_DAYS
    )
    ...
    stats["pct_long"] = (
        stats["long_lived"] / stats["total"] * 100
    )
    ...
    ax.set_xlabel(
        f"% of Persistent Cookies Lasting > {LONGTERM_THRESHOLD_DAYS} Days",
        fontsize=14
    )

    ax.set_title(
        f"Top {top_n} Websites by Share of Long-Lived Cookies",
        fontsize=16,
        pad=15
    )
