...
Compares browsers within a country.

Outputs:
    browser_tracker_comparison.png
    browser_tracker_comparison.pdf
    browser_tracker_comparison_white.png
    browser_tracker_comparison_white.pdf
"""
...
def main():
    ...
    parser.add_argument(
        "--data",
        default=os.path.join(ROOT, "cookies_data"),
    )
    ...
    if "country" in cookies.columns:
        cookies = cookies[cookies["country"].str.lower() == args.country.lower()]

    print(f"Country={args.country} " f"Rows={len(cookies):,}")
    ...
    stem = f"browser_tracker_comparison_" f"{args.country.replace(' ', '_')}"
