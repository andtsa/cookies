...
def plot_candlestick(
    ...
    **median** and **mean** overlaid as two lines. ``xscale`` is "linear"
    ...
    if len(rows) < 2:
        print("  candlestick: not enough populated bins; lower --cs-min-n or --cs-bins.")
        return
    ...
    for r in rows:
        ...
        if r["q3"] - r["q1"] < 1e-9:  # zero-width IQR
            ...
            ax.add_patch(mpatches.Rectangle(
                (left, r["q1"]), right - left, r["q3"] - r["q1"],
                facecolor=body_c, edgecolor=DARK, lw=0.5, alpha=0.55, zorder=3,
            ))
    ...
    ax.plot(xs, med, color=ACCENT, lw=2.0, marker="o", ms=3, mec=BG, mew=0.5,
    ...
    ax.set_xlabel(
        f"Website Rank ({xscale} scale — most popular → least)", fontsize=11
    )
    ...
    handles = [
        mpatches.Patch(facecolor=body_c, edgecolor=DARK, alpha=0.55,
...
def main(data_dir, rank_csv, out_dir, kind, bins, bin_mode,
         cs_bins, cs_min_n, cs_smooth, cs_xscale):
    ...
    if "candlestick" in chosen:
        plot_candlestick(
            matched, out_dir, cs_bins=cs_bins, min_n=cs_min_n,
            smooth=cs_smooth, xscale=cs_xscale,
        )
